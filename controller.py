...
class Controller:
    ...
    def match_date_range(self,initial_date:str, final_date:str)->str:
        """
        input:
            initial_date: string format %d/%m/%Y
            final_date: string format %d/%m/%Y
        output: string data-json 
        """
        return self.client.search(index=self.__INDEX,query={"range":{"RecordTime":{"gte":get_timestamp(initial_date),"lte":get_timestamp(final_date)}}})
